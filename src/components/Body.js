...
const Body = () => {
  ...
  const [trendingData,setTrendingData] = useState([]);
    ...
    const func = (category)=>{
      var pathName;
      if(category[0]==="m"){
          pathName = "/"+ category.substring(6,14);
      }
      else if(category[0]==="w"){
          pathName = "/"+ category.substring(8,16);
      }
      else pathName = "/"+category;
      return pathName;
  }
  // const newPath = func(proudData[0]?.category);
  // console.log(newPath)
  return (
    <div>
      ...
      <div>
        ...
        }*/}</div>
       {
         !proudData ? <div>shimmer UI</div> :
           <div className='productDiv'>
            {proudData.map((data)=>{
              return  <Link className='linkTag' key={data.id} to={`${func(data.category)}/${data.id}`} > <ProductPageCard data={data}/></Link>
            })}
           </div>
       }
      </div>
         ...
         !trendingData ? <div>shimmer UI</div> :
           <div className='productDiv'>
            {trendingData.map((data)=>{
              return <Link key={data.id} className='linkTag' to={`${func(data.category)}/${data.id}`}><ProductPageCard data={data} /> </Link>
            })}
           </div>
       }
        </div>
      ...
      <div className='bodyClothingDiv' style={{marginBottom:'10px'}}>
        <div className='bodyClothingDivLeft' >
          ...
          <Link to="/jewelery" ><button className='bodyClothingBtn'>Shop Now  →</button></Link>
        </div>
